        ...
        <div className="mt-14 mb-5">
          ...
          <div className="flex w-[100%] h-[40vh] overflow-x-scroll">
            {info.details.seasons.map((s, i) => (
              <div key={i} className="min-w-[15%] m-2 bg-zinc-900 overflow-y-scroll">
                <img key={i}
                  className="w-full h-[55%] object-cover"
                  src={`https://image.tmdb.org/t/p/original/${s.poster_path} alt="" `}
                />
                <div  className="p-4">
                  <h1 key={i} className="text-white font-semibold text-lg">{s.name}</h1>
                  {s.overview && (
                    <p key={i} className="text-white text-sm  mt-2">
                      {s.overview.slice(0, 40)}...
